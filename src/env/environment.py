...
class SP500TradingEnv(gym.Env):
    ...
    def step(self, action):
        # Check termination BEFORE trying to access next day's data
        # We need current_step + 1 to exist for reward calculation
        if self.current_step >= len(self.df) - 1:
            # Episode is over - return terminal state
            current_price = self.df.iloc[self.current_step]["Close"]
            current_val = self.balance + (self.shares_held * current_price)
            return np.zeros(8, dtype=np.float32), 0, True, False, {"net_worth": current_val}
        ...
        # 3. Calculate Net Worth using NEW Price
        ...
        # 5. Calculate Reward (Differential Sharpe Ratio)
    ...
    def _take_action(self, action, current_price):
        ...
        # 1 = BUY
        if action_type == 1:
            # More conservative position sizing to reduce volatility
            # Use 50% of available cash instead of 90%
            total_possible = self.balance * 0.5
            shares_to_buy = int(total_possible // current_price)
    ...
    def _calculate_dsr(self, current_val, prev_val):
        ...
        if prev_val <= 0 or current_val <= 0:
            return -1.0  # Penalize invalid states

        # Log Returns for stability
        R_t = np.log(current_val / prev_val)

        # Handle NaN/Inf from log
